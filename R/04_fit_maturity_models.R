...
data_global_sex <- data |>
  ...
  group_by(species_clean) |>
  filter(
    sum(sex == "M" & mature == 0) >= 15, sum(sex == "M" & mature == 1) >= 15,
    sum(sex == "F" & mature == 0) >= 15, sum(sex == "F" & mature == 1) >= 15
  ) |>
...
data_global_sex_period <- data |>
  filter(period != "2025-2029", sex %in% c("F", "M")) |>
  group_by(species_clean, period) |>
  filter(
    sum(sex == "M" & mature == 0) >= 15, sum(sex == "M" & mature == 1) >= 15,
    sum(sex == "F" & mature == 0) >= 15, sum(sex == "F" & mature == 1) >= 15
...
data_stock_comb_period <- data |>
  filter(period != "2025-2029", str_detect(species_stock, "\\.27\\.")) |>
