# 04_check_maturity_codes.R
#
# Validates uncertain maturity code boundaries by exploiting ICES areas where
# the same species is recorded with multiple scales simultaneously.
# Data filtered to spawning / pre-spawning season (same logic as 03).
#
# Uncertain boundaries tested (current → alternative):
#   SMSF Ba       : 0 (immature) → 1 (mature)
#   Roman full II : 0 (immature) → 1 (mature)    [scale goes to VII+]
#   Roman trunc II: 1 (mature)   → 0 (immature)  [scale max = VI]
#   Numeric 2     : 1 (mature)   → 0 (immature)
#
# Reference (unambiguous) scales: R1, R2, M6, IM
#
# Strategy:
#   Dataset A  areas where an uncertain scale co-occurs with a reference scale
#              → compare uncertain-scale L50 (current vs alt.) to reference L50
#   Dataset B  areas where multiple uncertain scales co-occur but no reference
#              → compare uncertain-scale L50s to each other across scenarios
#
# Outputs:
#   output/04_check_maturity/L50_vs_reference.png   — Dataset A
#   output/04_check_maturity/L50_multi_uncertain.png — Dataset B
...
# One row per species × country × scale; years packed as a list column.
...
# for each uncertain scale, the only scenarios that change its L50
...
# Per-scale display names, palette, and explicit boundary descriptions for plots.
scale_display <- c(
  "SMSF"        = "SMSF",
...
# For every scale × scenario pair: what is the exact boundary decision?
boundary_labels <- tribble(
  ...
  mutate(
    assignment_type = if_else(scenario == "current", "current", "alternative")
  )
...
# For each species × country, define which scales to compare.
# M6 is always included and serves as the unambiguous reference.
# Adjust based on scale_overview if a species/country lacks certain scales.
...
# linetype / shape keyed by boundary label for plot B
...
# ── 11. Plot A: L50 vs M6 — color by scale, shape by assignment ──────────────
...
ggplot(plot_data_a,
  ...
  labs(
    x = NULL, y = "L50 − L50 (M6)  [cm]"
  )

ggsave(here('output/supp/L50_vs_M6.png'), width = 7, height = 9,
       dpi = 450)
cat("Saved L50_vs_M6.png\n")

# ── 12. Plot B: L50 per year — color by scale, linetype + shape by assignment ─
...
ggsave(here('output/supp/L50_comparison_by_year.png'), width = 10, height = 9,
       dpi = 450)
...
cat("\n=== Best scenario per species × country × area × scale (smallest |dev| from M6) ===\n")
uncert_pooled %>%
